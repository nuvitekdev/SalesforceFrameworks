...
export default class SurveyCreator extends LightningElement {
  ...
  updatePathClasses() {
    ...
    setTimeout(() => {
      ...
      if (container) {
        ...
        const existingClasses = Array.from(container.classList).filter(
          (cls) => cls.includes("step-") && cls.includes("-mode")
        );
        ...
        pathItems.forEach((item, index) => {
          ...
          item.setAttribute(
            "aria-current",
            displayStep === this.currentStep ? "step" : "false"
          );
          item.setAttribute(
            "aria-selected",
            displayStep === this.currentStep ? "true" : "false"
          );
  ...
  } // Created Survey, show ID
  get isStep6() {
    return this.currentStep === 6;
  } // Sent Confirmation Step

  get step1Title() {
    return "Step 1: Survey Details";
  }
  get step2Title() {
    return "Step 2: Build Questions";
  }
  get step3Title() {
    return "Step 3: Add Recipients";
  }
  get step4Title() {
    return "Step 4: Review";
  }
  get step5Title() {
    return "Step 5: Survey Created";
  }
  get step6Title() {
    return "Survey Sent!";
  }
  ...
  } // Don't show on first or confirmation step
  get showNextButton() {
    return this.currentStep < 4;
  } // Show until Review step
  get showSaveButton() {
    return this.currentStep === 4;
  } // Only on Review step
  get showSendButton() {
    return this.currentStep === 5;
  } // Only on Created step
  get showCreateAnotherButton() {
    return this.currentStep === 6;
  } // Only on Sent Confirmation step
  get showNavButtons() {
    return this.currentStep <= 4;
  ...
  get isViewingSurveys() {
    return this.currentMode === "view" && !this.selectedSurveyData;
  }
  ...
  get createModeVariant() {
    return this.currentMode === "create" ? "brand" : "neutral";
  }
  get viewModeVariant() {
    return this.currentMode === "view" ? "brand" : "neutral";
  }
  ...
  get questionsWithComputedFields() {
    return this.questions.map((q) => {
      ...
      const showOptions =
        q.type &&
        ["Multiple Choice - Single", "Multiple Choice - Multiple"].includes(
          q.type
        );
  ...
  removeQuestion(event) {
    ...
    if (isNaN(questionIndexToRemove)) {
      this.showToast(
        "Error",
        "Could not determine which question to remove.",
        "error"
      );
      return;
    }
  ...
  addAnswerOption(event) {
    ...
    if (isNaN(questionIndex)) {
      this.showToast(
        "Error",
        "Could not determine which question to add an option to.",
        "error"
      );
      return;
    }
    ...
    const optionsCount = questions[questionIndex].answerOptions
  ...
  removeAnswerOption(event) {
    ...
    question.answerOptions = question.answerOptions.map((opt, oIndex) => ({
      ...opt,
    ...
    if (this.currentStep === 1) {
      ...
      if (!hasQuestions) {
        ...
        for (let i = 0; i < this.questions.length; i++) {
          ...
          if (
            q.type.startsWith("Multiple Choice") &&
            (!q.answerOptions || q.answerOptions.length < 2)
          ) {
            isValid = false;
  ...
  async handleSaveSurvey() {
    ...
    try {
      ...
      const cleanSurvey = {
        ...
        Welcome_Message__c: sanitizeString(
          this.survey.Welcome_Message__c || ""
        ),
        Thank_You_Message__c: sanitizeString(
          this.survey.Thank_You_Message__c || ""
        ),
      ...
      try {
        ...
        throw new Error(
          "Failed to convert survey data to JSON: " + jsonError.message
        );
      }
      ...
      this.showToast(
        "Success",
        "Survey created successfully! You can now send invitations.",
        "success"
      );
      ...
      this.showToast(
        "Error",
        `Failed to save survey: ${errorMessage}`,
        "error"
      );
    } finally {
  ...
  async handleSendEmails() {
    ...
    if (emailList.length === 0) {
      this.showToast(
        "Validation Error",
        "Please provide at least one valid recipient email address.",
        "error"
      );
      return;
    }
    ...
    if (!this.publicSiteUrl || this.publicSiteUrl.trim() === "") {
      this.showToast(
        "Configuration Error",
        "A valid Public Site URL is required.",
        "error"
      );
      return;
    }
    if (!this.publicSiteUrl.toLowerCase().startsWith("http")) {
      this.showToast(
        "Configuration Error",
        "Public Site URL must start with http:// or https://.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      this.showToast(
        "Success",
        `${this.sentCount} invitations sent successfully!`,
        "success"
      );
      ...
      this.showToast(
        "Error",
        `Failed to send survey: ${errorMessage}`,
        "error"
      );
  ...
  async loadUserSurveys() {
    ...
    try {
      ...
      this.error =
        this.reduceErrors(error).join(", ") || "Error loading surveys";
    } finally {
  ...
  async handleViewSurveyResponses(event) {
    ...
    try {
      ...
      this.error =
        this.reduceErrors(error).join(", ") || "Error loading survey responses";
    } finally {
  ...
  copyDirectLinkToClipboard() {
    if (!this.directSurveyLink || this.directSurveyLink.includes("Configure")) {
      this.showToast(
        "Error",
        "Please configure the public site URL first",
        "error"
      );
      return;
    }
