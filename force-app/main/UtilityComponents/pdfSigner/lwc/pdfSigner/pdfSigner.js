...
export default class PdfSigner extends LightningElement {
    ...
    if (!this.fontLoaded) {
      try {
        const fontFace = new FontFace(
          "SignatureFont",
          `url(${SIGNATURE}/GreatVibes-Regular.ttf)`
        );
  ...
  initializeCanvas() {
    setTimeout(() => {
      ...
      const containerWidth =
        this.canvasElement.parentElement?.clientWidth || 400;
    ...
    iframe.onload = () => {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          const toolbarElements = iframe.contentDocument.querySelectorAll(
            "#toolbar, #toolbarContainer, .toolbar"
          );
        ...
        console.error(
          "Could not modify iframe content due to same-origin policy",
          e
        );
      }
    };
  ...
  async analyzePdfOrientation() {
    ...
    try {
      ...
      console.log(
        `PDF orientation: ${this.isPdfLandscape ? "Landscape" : "Portrait"}`
      );
      console.log(
        `PDF dimensions: ${width} x ${height}, Aspect Ratio: ${height / width}`
      );
  ...
  placeSignatureOverlay(offsetX, offsetY) {
    ...
    const placementId =
      "sig-" + Date.now() + "-" + Math.round(Math.random() * 1000);
    ...
    if (isTextSignature) {
      ...
      const aspectRatio =
        this.signatureImage.height / this.signatureImage.width;
    ...
    sigWrapper.addEventListener(
      "mousedown",
      this.startDragSignature.bind(this)
    );
  ...
  handleDragOver(event) {
    ...
    event.stopPropagation();
    const uploadContainer = this.template.querySelector(
  ...
  startResizeSignature(event, sigElement, resizeHandle) {
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      if (this.pdfDoc) {
        ...
        const index = this.placedSignatures.findIndex(
          (p) => p.id === placementId
        );
        if (index >= 0) {
  ...
  async handleSavePdf() {
    ...
    if (this.placedSignatures.length === 0) {
      this.showToast(
        "Warning",
        "No signature placed on the document",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      const pageIndices = Object.keys(signaturesByPage).map((idx) =>
        parseInt(idx, 10)
      );
      ...
      for (const pageIndex of pageIndices) {
        try {
          ...
          console.log(
            `Processing page ${pageIndex} - Size: ${pageWidth}x${pageHeight}`
          );
          ...
          for (const placement of signaturesByPage[pageIndex]) {
            try {
              console.log(
                `Placing signature at: x=${placement.x}, y=${placement.y}, w=${placement.width}, h=${placement.height}`
              );
              ...
              console.error(
                `Error placing signature on page ${pageIndex}:`,
                err
              );
      ...
      const fileName = this.pdfFilename
        ? this.pdfFilename.replace(/\.pdf$/i, "_signed.pdf")
        : "SignedDocument.pdf";
      ...
      this.showToast(
        "Error",
        "Error saving PDF: " + (error.message || JSON.stringify(error)),
        "error"
      );
    } finally {
    ...
    try {
      ...
      for (let i = 0; i < pageCount; i++) {
        ...
        overlay.addEventListener("drop", (event) =>
          this.handleDropOnPage(event, i)
        );
        overlay.addEventListener("dblclick", (event) =>
          this.handleOverlayDoubleClickOnPage(event, i)
        );
  ...
  placeSignatureOnPage(offsetX, offsetY, pageIndex) {
    ...
    console.log(
      `Placed signature on page ${pageIndex + 1} at PDF coordinates:`,
      pdfX,
      pdfY
    );
  }
  ...
  updateContainerForCurrentPage() {
    ...
    try {
      ...
      console.log(
        `Container updated for page ${pageIndex + 1}: ${width}x${height}, ratio: ${aspectRatio.toFixed(2)}%`
      );
  ...
  triggerFileInput() {
    const nativeFileInput = this.template.querySelector(
    ...
    if (nativeFileInput) {
      ...
      console.error(
        'Could not find native input element with selector: input[type="file"].file-input-hidden'
      );
    }
  }
  ...
  updatePathClasses() {
    ...
    setTimeout(() => {
      ...
      console.log(
        `Found ${pathItems.length} path items for step: ${this.currentStep}`
      );
   ...
   */
  async cropSignature(source, padding = 2) {
    // Reduced padding
      ...
      img.onload = () => {
        ...
        try {
          ...
          for (let y = 0; y < canvas.height; y++) {
            for (let x = 0; x < canvas.width; x++) {
              ...
              if (data[alphaIndex] > 10) {
                // Use a threshold > 0 for anti-aliasing
          ...
          if (minX > maxX || minY > maxY) {
            console.warn(
              "Cropping failed: No content found or image is transparent."
            );
          ...
          console.log(
            `Cropped signature from ${img.width}x${img.height} to ${contentWidth}x${contentHeight}`
          );
          resolve({
          ...
          resolve({
            dataUrl:
              source instanceof HTMLCanvasElement ? source.toDataURL() : source,
            width: img.width,
            height: img.height
          });
        }
  ...
  async handleSignatureConfirmation() {
    ...
    if (
      (this.isDraw && (!this.signaturePad || this.signaturePad.isEmpty())) ||
      (this.isType && (!this.signatureText || this.signatureText.trim() === ""))
    ) {
      this.showToast(
        "Error",
        "Please create a signature before proceeding",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      this.showToast(
        "Error",
        "Could not process signature. Please try again.",
        "error"
      );
    }
  }
