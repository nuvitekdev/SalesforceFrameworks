({
    ...
    checkForNavigation: function(component) {
        // Initialize console listener on first run
        if (!component.get("v.consoleListenerSetup")) {
            component.set("v.consoleListenerSetup", true);
            this.setupConsoleListener(component);
        }
        
        // Check if the navBar has been loaded
        var navBar = component.get("v.navBar");
        var navBarElement = document.querySelector('.headerBanner .forceCommunityGlobalNavigation');
        
        if (navBar && navBar.length > 0 && navBarElement) {
            // Navigation is loaded and DOM element is rendered
            window.setTimeout(
            ...
            window.setTimeout(
                $A.getCallback(function() {
                    self.checkForNavigation(component);
                }), 300
            );
        }
    },
