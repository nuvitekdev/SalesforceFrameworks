...
export default class SFL_FileUpload extends NavigationMixin(LightningElement) {
  ...
  connectedCallback() {
    ...
    window.setTimeout(() => {
      ...
      if (
        this.isCommunity &&
        this.template.querySelector("lightning-file-upload")
      ) {
        try {
          ...
          console.error(
            "Error initializing file uploader in community:",
            error
          );
        }
      }
  ...
  get acceptedFileFormats() {
    try {
      return this.acceptedFormats
        ? this.acceptedFormats.split(",").map((format) => format.trim())
        : null;
    } catch (error) {
    ...
    try {
      ...
      if (this.isFlow) {
        try {
          ...
          const uploadCompleteEvt = new FlowAttributeChangeEvent(
            "uploadComplete",
            true
          );
          this.dispatchEvent(uploadCompleteEvt);

          const fileNamesEvt = new FlowAttributeChangeEvent(
            "fileNames",
            this.fileNames
          );
  ...
  handleError(event) {
    ...
    const errorMessage =
      event.detail?.message || "An error occurred during file upload";
  ...
  renderedCallback() {
    ...
    if (!this._renderedOnce) {
      ...
      if (this.isFlow) {
        ...
        const mainContainer = this.template.querySelector(
