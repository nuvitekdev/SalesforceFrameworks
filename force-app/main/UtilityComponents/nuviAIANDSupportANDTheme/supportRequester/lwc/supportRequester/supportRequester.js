 ...
 */
export default class SupportRequester extends NavigationMixin(
  LightningElement
) {
  ...
  @api defaultFaqItems =
    '[{"question":"How do I submit a case?","answer":"Fill out the form, add a recording if needed, and click Submit."},{"question":"How long can my recording be?","answer":"Recordings can be up to 5 minutes long by default."}]';
  ...
  get showScreenshotOption() {
    return (
      ...
      (this.effectiveCaptureMode === "screenshot" ||
        this.effectiveCaptureMode === "both")
    );
  }

  get showMediaTypeTabs() {
    return (
      this.enableScreenRecording &&
      this.enableScreenshot &&
      this.effectiveCaptureMode === "both"
    );
  }
  ...
  applyDynamicStyles() {
    ...
    this.template
      .querySelector(".support-requester-container")
      .appendChild(style);
  }
  ...
  handleSubmitCase() {
    ...
    createSupportCase({
      ...
      .then((caseId) => {
        ...
        if (
          (this.showPreview && this.recordedChunks.length > 0) ||
          (this.showScreenshot && this.screenshotUrl)
        ) {
        ...
        .then(() => {
          ...
          if (recordingUploadStarted && screenshotUploadStarted) {
            this.showSuccessToast(
              "Case Created",
              "Your case and media attachments were successfully submitted."
            );
          } else if (recordingUploadStarted) {
            this.showSuccessToast(
              "Case Created",
              "Your case and screen recording were successfully submitted."
            );
          } else if (screenshotUploadStarted) {
            this.showSuccessToast(
              "Case Created",
              "Your case and screenshot were successfully submitted."
            );
          }
          this.finalizeCaseCreation();
        })
  ...
  startRecording() {
    ...
    this.prepareRecording()
      ...
      .catch((error) => {
        ...
        this.errorMessage =
          "Screen recording permission denied. Please allow screen sharing to record your issue.";
      });
  }
        ...
        .then((screenStream) => {
          ...
          navigator.mediaDevices
            ...
            .then((audioStream) => {
              ...
              const videoElement =
                this.template.querySelector(".video-element");
              if (videoElement) {
            ...
            .catch((error) => {
              ...
              console.warn(
                "Microphone access denied, falling back to screen only:",
                error
              );
    ...
    this.prepareScreenCapture()
      ...
      .catch((error) => {
        ...
        this.screenshotErrorMessage =
          "Screen capture permission denied. Please allow screen sharing to capture your screen.";
      });
  }
