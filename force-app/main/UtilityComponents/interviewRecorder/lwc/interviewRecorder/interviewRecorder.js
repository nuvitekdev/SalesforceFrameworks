...
export default class InterviewRecorder extends LightningElement {
    ...
    applyCustomStyles() {
        ...
        root.style.setProperty('--primary-gradient', `linear-gradient(90deg, ${this.primaryColor} 0%, ${this.accentColor} 100%)`);
    ...
    get recordButtonDisabled() {
        return this.isProcessing || this.showCountdown || (!this.streamActive && !this.recording);
    }
    ...
    async initializeCamera() {
        try {
            ...
            this.errorMessage = 'Cannot access camera or microphone. Please ensure you have granted permission.';
        }
    }
    ...
    startRecordingProcess() {
        ...
        if (!this.effectiveRecordId) {
            this.showToast('Error', 'A Record ID is required to save the recording. Please configure this component with a valid Record ID.', 'error');
            return;
        }
    ...
    async submitRecording() {
        ...
        try {
            ...
            if (this.template.querySelector('lightning-flow-support')) {
                const attributeChangeEvent = new FlowAttributeChangeEvent('recordingCompleted', true);
                this.dispatchEvent(attributeChangeEvent);
            }
            
            // Show success message
            this.showToast('Success', 'Interview recording saved successfully', 'success');
            
            // Reset the recorder
            this.resetRecorder();
            
            // Dispatch event to notify parent component
            this.dispatchEvent(new CustomEvent('recordingsaved', {
                detail: { success: true }
            }));
