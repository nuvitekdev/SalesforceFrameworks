...
export default class LicenseVisualizerTool extends LightningElement {
  ...
  @track licenseColumns = [
    ...
    {
      label: "Utilization",
      fieldName: "utilizationPercentage",
      type: "percent",
      sortable: true,
  ...
  @track apiUsageColumns = [
    ...
    {
      label: "API Calls (24h)",
      fieldName: "apiCalls24h",
      type: "number",
      sortable: true
    },
    {
      label: "API Calls (7d)",
      fieldName: "apiCalls7d",
      type: "number",
      sortable: true
    },
    {
      label: "API Calls (30d)",
      fieldName: "apiCalls30d",
      type: "number",
      sortable: true
    },
    {
  ...
  @track profileActivityColumns = [
    ...
    {
      label: "% of Total",
      fieldName: "userPercent",
      type: "percent",
      sortable: true
    },
    {
      label: "Active Users",
      fieldName: "activeUsersCount",
      type: "number",
      sortable: true
    },
    {
      label: "% Active",
      fieldName: "activeUsersPercent",
      type: "percent",
      sortable: true,
  ...
  isChartContainerReady() {
    try {
      ...
      if (this.isLicenseTab) {
        ...
        return (
          !!this.template.querySelector(".login-trend-chart") ||
          !!this.template.querySelector(".heatmap-chart")
        );
      } else if (this.isFeatureTab) {
        return (
          !!this.template.querySelector(".permission-chart") ||
  ...
  initializeChartJs() {
    ...
    const loadingTimeout = setTimeout(() => {
      if (!loadingPromiseResolved) {
        ...
        this.handleError(
          "Timeout loading chart libraries. Try refreshing the page."
        );
      }
    ...
    Promise.all([
      ...
      .catch((error) => {
        ...
        this.handleError(
          "Error loading chart libraries: " + this.reduceError(error)
        );
      });
  }
  ...
  initializeChartsForCurrentTab() {
    ...
    setTimeout(() => {
      try {
        ...
        this.handleError(
          "Error initializing charts: " + this.reduceError(error)
        );
      }
  ...
  renderLicenseChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering license chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating license chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderUserActivityCharts() {
    ...
    if (
      !this.userActivityData.heatmapData ||
      !this.userActivityData.geoData ||
      !this.userActivityData.loginMethodData
    ) {
      console.warn(
        "Some user activity data is missing:",
        "heatmapData:",
        !!this.userActivityData.heatmapData,
        "geoData:",
        !!this.userActivityData.geoData,
    ...
    const attemptRendering = (attempt = 0) => {
      if (attempt >= 3) {
        ...
        this.handleError(
          "Some charts could not be rendered due to missing containers"
        );
        return;
      }
      ...
      try {
        ...
        this.handleError(
          "Error rendering user activity charts: " + this.reduceError(error)
        );
      }
    };
  ...
  renderLoginTrendChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering login trend chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating login trend chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderLoginHeatmap() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering heatmap: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating heatmap chart: " + this.reduceError(error)
      );
  ...
  renderGeoDistribution() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering geo distribution chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating geo distribution chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderLoginMethodChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering login method chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating login method chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderFeatureAdoptionCharts() {
    ...
    setTimeout(() => {
      try {
        ...
        if (
          this.featureAdoptionData.permissionSetData &&
          this.featureAdoptionData.permissionSetData.length > 0
        ) {
          this.renderPermissionSetChart();
        }

        if (
          this.featureAdoptionData.profileData &&
          this.featureAdoptionData.profileData.length > 0
        ) {
          this.renderProfileChart();
        }

        if (
          this.featureAdoptionData.objectUsageData &&
          this.featureAdoptionData.objectUsageData.length > 0
        ) {
          this.renderObjectUsageChart();
        }

        if (
          this.featureAdoptionData.permissionSetByProfile &&
          this.featureAdoptionData.permissionSetByProfile.length > 0
        ) {
          this.renderPermProfileChart();
        }
      } catch (error) {
        this.handleError(
          "Error rendering feature adoption charts: " + this.reduceError(error)
        );
      }
  ...
  renderPermissionSetChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering permission set chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating permission set chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderProfileChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering profile chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating profile chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderObjectUsageChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering object usage chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    const sortedData = [...objectData].sort(
    ...
    try {
      ...
      this.handleError(
        "Error creating object usage chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderPermProfileChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering permission set by profile chart: Canvas context could not be obtained"
      );
      return;
    }

    const permProfileData =
      this.featureAdoptionData.permissionSetByProfile || [];
    ...
    const profiles = [
      ...new Set(permProfileData.map((item) => item.profileName))
    ];
    const permSets = [
      ...new Set(permProfileData.map((item) => item.permissionSetName))
    ];

    // Create datasets for each profile
    const datasets = profiles.map((profile, index) => {
      const profileData = permProfileData.filter(
        (item) => item.profileName === profile
      );

      // Map data to match permission set order
      const data = permSets.map((permSet) => {
        const match = profileData.find(
          (item) => item.permissionSetName === permSet
        );
      ...
      return {
        ...
        data: data,
        backgroundColor:
          this.getChartColorPalette()[
            index % this.getChartColorPalette().length
          ],
        barPercentage: 0.8,
    ...
    try {
      ...
      this.handleError(
        "Error creating permission set by profile chart: " +
  ...
  renderApiUsageCharts() {
    ...
    setTimeout(() => {
      try {
        ...
        // Use a defensive approach - only render if we have actual data
        if (
          this.apiUsageData.trendData &&
          this.apiUsageData.trendData.length > 0
        ) {
          this.renderApiTrendChart();
        }

        if (
          this.apiUsageData.apiConsumers &&
          this.apiUsageData.apiConsumers.length > 0
        ) {
          this.renderApiConsumersChart();
        }

        if (
          this.apiUsageData.connectedApps &&
          this.apiUsageData.connectedApps.length > 0
        ) {
          this.renderConnectedAppsChart();
        }
      } catch (error) {
        this.handleError(
          "Error rendering API usage charts: " + this.reduceError(error)
        );
      }
    }, 250); // Longer timeout to ensure DOM is properly rendered
  }
  ...
  renderApiTrendChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering API trend chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating API trend chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderApiConsumersChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering API consumers chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating API consumers chart: " + this.reduceError(error)
      );
    }
  }
  ...
  renderConnectedAppsChart() {
    ...
    if (!ctx) {
      this.handleError(
        "Error rendering connected apps chart: Canvas context could not be obtained"
      );
      return;
    }
    ...
    try {
      ...
      this.handleError(
        "Error creating connected apps chart: " + this.reduceError(error)
      );
    }
  }
  ...
  loadLicenseData() {
    getLicenseData({
      ...
      .catch((error) => {
        this.handleError(
          "Error loading license data: " + this.reduceError(error)
        );
      });
  }
  ...
  loadUserActivityData() {
    getUserLoginActivity({
      ...
      .catch((error) => {
        this.handleError(
          "Error loading user activity data: " + this.reduceError(error)
        );
        this.isLoading = false;
      });
  }
  ...
  loadFeatureAdoptionData() {
    getFeatureAdoptionData()
      ...
      .catch((error) => {
        this.handleError(
          "Error loading feature adoption data: " + this.reduceError(error)
        );
      });
  }
  ...
  loadApiUsageData() {
    getApiUsageData({
      ...
      .catch((error) => {
        this.handleError(
          "Error loading API usage data: " + this.reduceError(error)
        );
      });
  }
  ...
  handleExportClick() {
    ...
    exportData({ exportType, filters: JSON.stringify(filters) })
      ...
      .catch((error) => {
        this.showToast(
          "Error",
          "Failed to export data: " + this.reduceError(error),
          "error"
        );
      });
  }
  ...
  viewInactiveUsers(licenseRow) {
    ...
    if (!this.selectedLicense.inactiveSavings) {
      this.selectedLicense.inactiveSavings = (
        this.selectedLicense.inactiveLicenses *
        this.selectedLicense.costPerActiveUser
      ).toFixed(2);
    }
    ...
    if (this.licenseData.sharingOpportunities) {
      ...
      if (sharingOpp) {
        ...
        this.showToast(
          ...
          "No license sharing opportunities identified for this license type.",
          "info"
        );
      }
    } else {
      this.showToast(
        "No Sharing Opportunities",
        "No license sharing opportunities identified.",
        "info"
      );
    }
  }
  ...
  viewSharingGroupUsers(event) {
    ...
    const selectedGroup = this.licenseSharingDetails.find(
    ...
    getLicenseSharingOpportunities({ licenseType: licenseType })
      .then((result) => {
        ...
        if (data.sharingGroups && Array.isArray(data.sharingGroups)) {
          this.licenseSharingDetails = data.sharingGroups.map(
            (group, index) => {
  ...
  loadAccountLockouts() {
    ...
    getAccountLockouts({ timeScale: this.selectedTimeScale })
      ...
      .catch((error) => {
        ...
        console.error(
          "Error loading account lockout data: " + this.reduceError(error)
        );
      });
  }
  ...
  viewLockoutDetails() {
    if (this.lockoutDetails && this.lockoutDetails.length > 0) {
      ...
      this.showToast(
        "No Lockouts Found",
        "No account lockouts were detected in the selected time period.",
        "info"
      );
    }
  }
  ...
  exportLockoutDetails() {
    ...
    exportData({
      ...
      .catch((error) => {
        this.showToast(
          "Error",
          "Failed to export lockout data: " + this.reduceError(error),
          "error"
        );
      });
  }
  ...
  getCanvasContext(selector) {
    try {
      ...
      if (rect.width === 0 || rect.height === 0) {
        ...
        console.warn(
          `Container ${selector} has zero dimensions, setting minimum size`
        );
      }
      ...
      console.log(
        `Canvas created successfully for ${selector} with dimensions ${containerWidth}x${containerHeight}`
      );
