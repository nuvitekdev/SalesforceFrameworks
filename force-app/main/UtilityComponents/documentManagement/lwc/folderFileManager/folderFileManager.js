...
export default class FolderFileManager extends NavigationMixin(
  LightningElement
) {
  ...
  acceptedFormats =
    ".pdf,.png,.jpg,.jpeg,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv";
  maxFilesLimit = 10;
  ...
  get noFoldersMessageText() {
    return this.searchTerm
      ? "No folders found. Try a different search term."
      : "No folders found.";
  }
  ...
  get showUploadButton() {
    ...
    return this.showFiles && !this.searchTerm;
  ...
  get fileColumns() {
    ...
    if (this.allowDelete) {
      ...
      columns.push({
        type: "action",
        typeAttributes: {
          rowActions: [{ label: "Download", name: "download" }],
          menuAlignment: "right"
        },
        initialWidth: 80
      });
  ...
  applyCustomStyling() {
    try {
      ...
      const styleContainer = this.template.querySelector(
  ...
  initializeComponent() {
    ...
    this.loadFolderStructure()
      ...
      .catch((error) => {
        this.displayToast(
          "Error",
          "Error loading folder structure: " + this.getErrorMessage(error),
          "error"
        );
        this.isLoading = false;
      });
  ...
  setupFolderHierarchy() {
    ...
    for (let path of this.folderStructure) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        const existingFolder = this.folderHierarchy.find(
          (f) => f.fullPath === currentPath
        );
    ...
    const defaultFolder = this.folderHierarchy.find(
      (f) => f.name === "Default"
    );
    if (!defaultFolder) {
  ...
  showTopLevelFolders() {
    ...
    getFolders({ recordId: this.recordId })
      .then((result) => {
        ...
        const topLevelFolders = this.folderHierarchy.filter(
      ...
      .catch((error) => {
        this.displayToast(
          "Error",
          "Error loading folders: " + this.getErrorMessage(error),
          "error"
        );
        this.isLoading = false;
      });
  ...
  loadFolderFiles(folderPath) {
    ...
    getFolderFiles({ recordId: this.recordId, folderName: folderPath })
      ...
      .catch((error) => {
        this.displayToast(
          "Error",
          "Error loading files: " + this.getErrorMessage(error),
          "error"
        );
        this.isLoading = false;
      });
  ...
  updateBreadcrumbs(folderPath, isSearchResults = false, searchTerm = "") {
    ...
    this.breadcrumbs.push({
      ...
      name: "",
      computedClass:
        folderPath === "" && !isSearchResults ? "breadcrumb-current" : ""
    });
  ...
  downloadFile(file) {
    try {
      ...
      if (!fileId) {
        this.displayToast(
          "Error",
          "File ID is missing. Cannot download file.",
          "error"
        );
        return;
      }
      ...
      this.displayToast(
        "Error",
        "Error downloading file: " + this.getErrorMessage(error),
        "error"
      );
    }
  }
    ...
    if (confirm("Are you sure you want to delete this file?")) {
      ...
      deleteFile({ contentDocumentId: file.ContentDocumentId })
        ...
        .catch((error) => {
          this.displayToast(
            "Error",
            "Error deleting file: " + this.getErrorMessage(error),
            "error"
          );
          this.isLoading = false;
        });
  ...
  performSearch() {
    ...
    const searchFilesPromise = getFolderFiles({
      ...
      .then((result) => {
        ...
        allFiles.sort((a, b) => {
          if (a.ContentModifiedDate && b.ContentModifiedDate) {
            return (
              new Date(b.ContentModifiedDate) - new Date(a.ContentModifiedDate)
            );
          }
          return 0;
        });
      ...
      .catch((error) => {
        this.displayToast(
          "Error",
          "Error searching files: " + this.getErrorMessage(error),
          "error"
        );
        return [];
      });
    ...
    const searchFoldersPromise = getFolders({ recordId: this.recordId })
      .then((result) => {
        try {
          ...
          const serverMatchingFolders = (result || [])
            .filter((folder) => {
          ...
          const hierarchyMatchingFolders = folderHierarchySearch().map(
            (folder) => {
              // Create fresh objects instead of modifying proxy objects
              return {
                id: folder.id || "",
      ...
      .catch((error) => {
        this.displayToast(
          "Error",
          "Error searching folders: " + this.getErrorMessage(error),
          "error"
        );
    ...
    Promise.all([searchFilesPromise, searchFoldersPromise])
      .then(([filteredFiles, filteredFolders]) => {
        ...
        const validFolders = filteredFolders.filter(
          (folder) =>
            folder &&
            folder.name &&
            typeof folder.name === "string" &&
      ...
      .catch((error) => {
        this.displayToast(
          "Error",
          "Error searching: " + this.getErrorMessage(error),
          "error"
        );
        this.isLoading = false;
      });
  ...
  processMultipleFiles(fileList) {
    ...
    if (remainingSlots <= 0) {
      this.displayToast(
        "Warning",
        `You can only upload up to ${this.maxFilesLimit} files at once.`,
        "warning"
      );
      return;
    }
    ...
    if (filesArray.length > remainingSlots) {
      this.displayToast(
        "Warning",
        `Only the first ${remainingSlots} files were selected due to the ${this.maxFilesLimit} file limit.`,
        "warning"
      );
    }
  ...
  processFile(file) {
    if (file) {
      ...
      reader.onload = () => {
        ...
        const fileId =
          "file_" + Date.now() + "_" + Math.round(Math.random() * 1000);
  ...
  handleFileChange(event) {
    try {
      ...
      this.displayToast(
        "Error",
        "Error selecting files: " + this.getErrorMessage(error),
        "error"
      );
    }
  }
      ...
      if (this.currentFolderPath) {
        ...
        getFolderFiles({
          ...
          .catch((error) => {
            this.displayToast(
              "Error",
              "Unable to refresh files: " + this.getErrorMessage(error),
              "error"
            );
            this.isLoading = false;
        ...
        getFolders({
          ...
          .catch((error) => {
            this.displayToast(
              "Error",
              "Unable to refresh folders: " + this.getErrorMessage(error),
              "error"
            );
            this.isLoading = false;
  ...
  fixDropdownPositioning() {
    ...
    const actionCells = this.template.querySelectorAll(
