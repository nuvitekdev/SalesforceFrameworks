    ...
    try {
      ...
      if (effectiveRecordId) {
        console.log(
          "Using record ID:",
          effectiveRecordId,
          this.recordId ? "(from API property or URL)" : "(from URL detection)"
        );
      } else {
        console.error(
          "Record ID is missing. This component can only be used on record pages or with a specified record ID."
        );
  ...
  navigateToFilePreview(contentDocumentId) {
    ...
    if (this.isPortalContext) {
      const file = this.files.find(
        (f) => f.ContentDocumentId === contentDocumentId
      );
      if (file) {
