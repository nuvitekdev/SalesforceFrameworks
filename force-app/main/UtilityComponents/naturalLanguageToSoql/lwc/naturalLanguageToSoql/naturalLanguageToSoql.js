...
export default class NaturalLanguageToSoql extends LightningElement {
  ...
  async handleRunQuery() {
    ...
    if (!this._objectList || this._objectList.length === 0) {
      this.handleError(
        "Please specify at least one object API name in the component configuration."
      );
      return;
    }
    ...
    try {
      console.log(
        `Running query across objects: ${this.objectApiNames} using LLM config: ${this.llmConfigName}`
      );
      ...
      if (
        ...
        response.enhancedQuery !== this.userQuery.toLowerCase()
      ) {
      ...
      if (
        ...
        console.log(
          "Processed results with recordUrl:",
          JSON.stringify(this.results[0])
        );
      } else {
        this.results = response.results || []; // Ensure results is an array
      }
      console.log(
        "Results assigned to datatable:",
        JSON.stringify(this.results, null, 2)
      );

      // Make a copy of columns and fix any fieldName issues if necessary
      this.columns = this.processColumnDefinitions(
        response.columns,
        response.primaryObject
      );
      console.log(
        "Columns assigned to datatable:",
        JSON.stringify(this.columns, null, 2)
      );
      ...
      if (this.results && this.results.length > 0) {
        ...
        this.showToast(
          "Query Successful",
          `Found ${this.resultCount} records`,
          "success"
        );
      } else {
        this.hasResults = false;
  ...
  handleCopyQuery() {
    ...
    try {
      ...
      this.handleError(
        "Clipboard functionality not available: " + error.message
      );
    }
  }
  ...
  async fetchDynamicExamples() {
    ...
    console.log(
      `Fetching dynamic examples for objects: ${this.objectApiNames} using LLM: ${this.llmConfigName}`
    );
    ...
    try {
      ...
      if (examples && examples.length > 0) {
        ...
        console.log(
          "Received dynamic examples:",
          JSON.stringify(this.dynamicExampleQueries)
        );
      } else {
      ...
      this.handleError(
        "Could not load dynamic examples: " + this.reduceErrors(error)
      );
  ...
  updateThemeVariables() {
    ...
    try {
      ...
      if (primaryHex && primaryHex.startsWith("#")) {
        ...
        const lighterHex = this.hslToHex(
          hsl.h,
          hsl.s,
        ...
        const headerElements =
          this.template.querySelectorAll(".header-section");
  ...
  generateGenericExampleQueries() {
    ...
    if (this._objectList.includes("Account")) {
      examples.push({
        id: "recentAccounts",
        query: `Show me recent ${this._objectList.includes("Account") ? "Accounts" : "records"}`
      });
    }
    ...
    if (this._objectList.includes("Contact")) {
      examples.push({
        id: "contactsQuery",
        query: "Find Contacts in California"
      });
      ...
      examples.push({
        id: "locationQuery",
        query: "Find records in a specific location"
      });
    }
    ...
    if (this._objectList.includes("Case")) {
      ...
      examples.push({
        id: "statusQuery",
        query: "Find records with specific status"
      });
    }
    ...
    if (examples.length < 4) {
      examples.push({
        id: "genericQuery",
        query: "Show records created this month"
      });
    }
  ...
  reduceErrors(errors) {
    ...
    if (
      errors.body &&
      Array.isArray(errors.body.pageErrors) &&
      errors.body.pageErrors.length > 0
    ) {
    ...
    if (errors.body && errors.body.fieldErrors) {
      ...
      for (const field in errors.body.fieldErrors) {
        if (
          Array.isArray(errors.body.fieldErrors[field]) &&
          errors.body.fieldErrors[field].length > 0
        ) {
          fieldErrors.push(
            ...errors.body.fieldErrors[field].map((e) => e.message)
          );
        }
      }
  ...
  processObjectApiNames() {
    if (this.objectApiNames && typeof this.objectApiNames === "string") {
      this._objectList = this.objectApiNames
        .split(",")
        .map((name) => name.trim())
  ...
  processColumnDefinitions(columnsFromServer, primaryObject) {
    ...
    return columnsFromServer.map((column) => {
      ...
      if (
        processedColumn.fieldName &&
        processedColumn.fieldName.includes(".")
      ) {
        ...
        if (
          parts.length === 2 &&
          parts[0].toLowerCase() === primaryObject.toLowerCase()
        ) {
          ...
          processedColumn.fieldName =
            standardFieldMap[fieldNameLower] || fieldName;

          console.log(
            `Processed column: ${column.fieldName} -> ${processedColumn.fieldName}`
          );
        }
