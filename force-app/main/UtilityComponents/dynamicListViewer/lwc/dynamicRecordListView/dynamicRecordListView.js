 ...
 */
export default class DynamicRecordListView extends NavigationMixin(
  LightningElement
) {
  ...
  @api objectApiName;
  /** Optional. Title to display at the top of the list view */
  @api listViewTitle;
  ...
  connectedCallback() {
    ...
    if (this.objectApiName && this.listViewFields) {
      ...
      console.warn(
        "DynamicRecordListView: listViewFields property is missing. Columns may not display correctly."
      );
      this.handleError(
        "Configuration Error: List view fields (listViewFields) are required but not specified."
      );
    }
  }
  ...
  processFieldConfiguration() {
    ...
    if (!this.objectApiName || !this.listViewFields) {
      this.handleError(
        "Configuration incomplete. Please provide both objectApiName and listViewFields."
      );
      return;
    }
    ...
    try {
      ...
      let fieldsList = this.listViewFields
        .split(",")
    ...
    getRecordAllFields({ objectApiName: objectApiName, recordId: recordId })
      .then((result) => {
        ...
        if (result && Array.isArray(result)) {
          ...
          const recordData = result.reduce(
            (acc, field) => {
          ...
          console.log("🔍 Built recordData:", {
            recordData: recordData,
            hasAttributes: !!recordData.attributes,
            attributesType: recordData.attributes?.type,
          ...
          console.log(
            "Detailed fields data:",
            JSON.stringify(this.detailedFieldsData)
          );
        } else {
      ...
      .catch((error) => {
        ...
        if (error.body && error.body.message) {
          ...
          if (
            ...
            errorMessage.includes("does not support query")
          ) {
  ...
  handleLookupNavigation(event) {
    ...
    if (this.effectiveLookupNavigationDisabled) {
      console.log(
        "Lookup navigation is disabled due to configuration or maximum depth reached"
      );
      return;
    }
    ...
    if (!objectApiName || objectApiName === "Name") {
      ...
      console.error(
        "Invalid object API name for lookup navigation:",
        objectApiName
      );
      this.handleError(
        "Cannot navigate to this record: Invalid or unsupported object type."
      );
      return;
    }
    ...
    if (this.selectedRecord) {
      ...
      console.log(
        ...
        JSON.stringify(this.navigationStack[this.navigationStack.length - 1])
      );
    }
  ...
  isValidRecord() {
    ...
    const hasId = this.selectedRecord?.Id;
    const hasType = this.selectedRecord?.attributes?.type || this.objectApiName;

    console.log("🔍 isValidRecord check:", {
    ...
    return hasSelectedRecord && hasId && hasType;
  }
  ...
  loadAllRelatedRecords() {
    ...
    return Promise.all(loadPromises)
      .then((results) => {
        ...
        console.log(
          "Loaded records for",
          this.relatedRecordsData.length,
          "related objects"
        );
      })
      .catch((error) => {
  ...
  loadRecordsForRelationship(relatedObject) {
    return this.fetchRelatedRecords(relatedObject.relationshipName).then(
      (records) => {
  ...
  loadFilesAndAttachments() {
    ...
    Promise.all([
      this.fetchRelatedRecordsWithFields(
        "ContentDocumentLinks",
        contentDocumentLinkFields
      ),
      ...
      .then(async ([contentLinks, classicAttachments, classicNotes]) => {
        ...
        if (contentLinks && contentLinks.length > 0) {
          ...
          const enrichedLinks =
            await this.enrichContentDocumentLinks(contentLinks);
        ...
        allFiles.sort(
          (a, b) => new Date(b.rawCreatedDate) - new Date(a.rawCreatedDate)
        );
  ...
  fetchRelatedRecordsWithFields(relationshipName, fields) {
    ...
    // Safely get the object type with fallback
    const objectType =
      this.selectedRecord?.attributes?.type || this.objectApiName;

    // Call the Apex method with individual parameters (not an object)
  ...
  async enrichContentDocumentLinks(contentLinks) {
    ...
    const documentIds = [
      ...new Set(contentLinks.map((link) => link.ContentDocumentId))
    ];
  ...
  formatFiles(fileLinks) {
    return fileLinks.map((link) => {
      ...
      if (contentDoc.Title) {
        ...
        contentDoc.LatestPublishedVersion &&
      ...
      return {
        ...
        fileSize: this.formatFileSize(
          contentDoc.ContentSize || contentDoc.Size || 0
        ),
        createdDate: contentDoc.CreatedDate
          ? this.formatDateTime(contentDoc.CreatedDate)
          : "",
    ...
    return attachments.map((attachment) => {
      ...
      return {
        ...
        createdDate: attachment.CreatedDate
          ? this.formatDateTime(attachment.CreatedDate)
          : "",
    ...
    return notes.map((note) => {
      return {
        ...
        createdDate: note.CreatedDate
          ? this.formatDateTime(note.CreatedDate)
          : "",
  ...
  handleChatterPostsClick() {
    if (!this.isValidRecord()) {
      console.log("❌ handleChatterPostsClick: Invalid record, aborting");
    ...
    Promise.allSettled([
      ...
      .then((results) => {
        ...
        try {
          ...
          if (
            activityHistories.status === "fulfilled" &&
            activityHistories.value &&
            activityHistories.value.length > 0
          ) {
            console.log(
              "Found",
              activityHistories.value.length,
              "activity histories"
            );
            allActivities.push(
              ...this.formatActivityRecords(activityHistories.value)
            );
          }

          // Process Open Activities (upcoming tasks/events)
          if (
            openActivities.status === "fulfilled" &&
            openActivities.value &&
            openActivities.value.length > 0
          ) {
            console.log(
              "Found",
              openActivities.value.length,
              "open activities"
            );
            allActivities.push(
              ...this.formatActivityRecords(openActivities.value, "Open")
            );
          }
          ...
          if (
            emailMessages.status === "fulfilled" &&
            emailMessages.value &&
            emailMessages.value.length > 0
          ) {
            console.log("Found", emailMessages.value.length, "email messages");
            allActivities.push(
              ...this.formatEmailMessages(emailMessages.value)
            );
          }

          // Process Case Comments (if this is a Case record)
          if (
            caseComments.status === "fulfilled" &&
            caseComments.value &&
            caseComments.value.length > 0
          ) {
          ...
          const uniqueActivities =
            this.removeDuplicateActivities(allActivities);
          ...
          this.handleError(
            processingError,
            "Error processing activity timeline data"
          );
          this.chatterPosts = [];
        }
      })
  ...
  fetchRelatedRecords(relationshipName) {
    console.log("🔍 fetchRelatedRecords called with:", {
      relationshipName: relationshipName,
      isValidRecord: this.isValidRecord(),
      selectedRecord: this.selectedRecord,
      selectedRecordKeys: this.selectedRecord
        ? Object.keys(this.selectedRecord)
        : null,
      hasAttributes: this.selectedRecord?.attributes ? true : false,
      attributesKeys: this.selectedRecord?.attributes
        ? Object.keys(this.selectedRecord.attributes)
        : null,
      objectApiName: this.objectApiName
    });
    ...
    if (!this.isValidRecord()) {
      console.log(
        "❌ fetchRelatedRecords: Invalid record, returning empty array"
      );
      return Promise.resolve([]);
    }
    ...
    const objectType =
      this.selectedRecord?.attributes?.type || this.objectApiName;
    console.log("🔍 fetchRelatedRecords: Using object type:", objectType);

    console.log(
      "🔍 fetchRelatedRecords: Calling Apex with individual parameters:",
      {
    ...
    return getRelatedRecords(
      ...
      .then((result) => {
        console.log(
          "🔍 fetchRelatedRecords: Apex returned for",
          relationshipName,
          ":",
          {
            ...
            firstRecordKeys: result?.[0] ? Object.keys(result[0]) : null
          }
        );
        return result;
      })
   ...
   */
  tryLoadFeedItems() {
    return this.fetchRelatedRecords("FeedItems").then((result) => {
  ...
  tryLoadContentDocuments() {
    return this.fetchRelatedRecords("ContentDocumentLinks").then((result) => {
      if (result && result.length > 0) {
        // Add files as a special type of activity
        const fileItems = result.map((link) => {
          return {
            id: link.Id,
            createdDate: link.SystemModstamp
              ? this.formatDateTime(link.SystemModstamp)
              : "",
            createdById: link.LastModifiedById,
            createdByName: "File Attachment",
            body:
              link.ContentDocument?.Description ||
              "File: " + (link.ContentDocument?.Title || "Untitled"),
            type: "File",
   ...
   */
  loadActivityHistory() {
    return this.fetchRelatedRecords("ActivityHistories").then((result) => {
      if (result && result.length > 0) {
        this.chatterPosts = [
          ...this.chatterPosts,
          ...this.formatActivityRecords(result)
        ];
        return true;
      }
   ...
   */
  tryLoadTasks() {
    return this.fetchRelatedRecords("Tasks").then((result) => {
      if (result && result.length > 0) {
        this.chatterPosts = [
          ...this.chatterPosts,
          ...this.formatActivityRecords(result)
   ...
   */
  tryLoadEvents() {
    return this.fetchRelatedRecords("Events").then((result) => {
      if (result && result.length > 0) {
        this.chatterPosts = [
          ...this.chatterPosts,
          ...this.formatActivityRecords(result)
  ...
  formatChatterPosts(posts) {
    return posts.map((post) => {
      const createdDate = post.CreatedDate
        ? this.formatDateTime(post.CreatedDate)
        : "";
  ...
  formatActivityRecords(activities, statusPrefix = "") {
    console.log("🔍 formatActivityRecords called with:", {
    ...
    return activities.map((activity, index) => {
      console.log(`🔍 Processing activity ${index}:`, {
        activity: activity,
        hasAttributes: !!activity.attributes,
        attributesType: activity.attributes?.type,
        activityKeys: Object.keys(activity || {})
      });

      const activityDate = activity.ActivityDate
        ? this.formatDate(activity.ActivityDate)
        : activity.CreatedDate || new Date().toISOString();
      ...
      // Safely get the activity type with fallback
      const activityType = activity.attributes?.type || "Activity";
      console.log(`🔍 Activity ${index} type resolved to:`, activityType);
      ...
      return {
        ...
        createdByName:
          activity.Owner?.Name || activity.CreatedBy?.Name || "User",
        body: activity.Description || activity.Comments || "",
        type: `${prefix}${activityType}`,
        title: `${subject} ${status}`.trim(),
        iconName: this.getActivityIcon(activityType)
      };
    });
  }
  ...
  updateCustomProperties() {
    ...
    if (container) {
      ...
      const extractRGB = (color) => {
        ...
        console.warn(
          `Unrecognized color format for RGB extraction: ${color}. Using default black.`
        );
        return "0, 0, 0";
      };
      ...
      container.style.setProperty(
        "--primary-color-rgb",
        extractRGB(this.primaryColor)
      );
      container.style.setProperty(
        "--accent-color-rgb",
        extractRGB(this.accentColor)
      );
    }
  }
  ...
  handleError(error, contextMessage = "An error occurred") {
    ...
    // Display a toast notification to the user - DISABLED
    // try {
    // this.dispatchEvent(
    //     new ShowToastEvent({
  ...
  get displayFields() {
    ...
    if (
      !this.detailedFieldsData ||
      !Array.isArray(this.detailedFieldsData) ||
      this.detailedFieldsData.length === 0 ||
      this.isLoadingRecordDetail
    ) {
    ...
    console.log(
      `Processing ${this.detailedFieldsData.length} fields for display`
    );
    ...
    this.detailedFieldsData.forEach((field) => {
      ...
      if (
        field.isReference &&
        (!field.referenceToObject || field.referenceToObject === "Name")
      ) {
        console.log(
          `Warning: Reference field ${field.apiName} has invalid object type: ${field.referenceToObject}`
        );
      ...
      const isAddress =
        this.isAddressField(field.apiName) ||
        (typeof field.value === "object" &&
          this.hasAddressComponents(field.value));
  ...
  sortFieldsForDisplay(fields) {
    ...
    const getWeight = (fieldName) => {
      ...
      if (contactFields.some((f) => lowerName.includes(f.toLowerCase())))
        return 10;
      if (addressFields.some((f) => lowerName.includes(f.toLowerCase())))
        return 15;
      if (statusFields.some((f) => lowerName.includes(f.toLowerCase())))
        return 20;
      ...
      if (dateFields.some((f) => lowerName.includes(f.toLowerCase())))
        return 80;
  ...
  formatFieldValue(field) {
    ...
    try {
      ...
      console.error(
        `Error formatting ${field.type} field ${field.apiName}: ${error.message}`
      );
      return String(field.value);
    }
  }
  ...
  isAddressField(fieldName) {
    ...
    return addressIndicators.some((indicator) =>
      lowerFieldName.includes(indicator)
    );
  }
  ...
  hasAddressComponents(obj) {
    ...
    return Object.keys(obj).some((key) =>
      addressComponents.some((component) =>
        key.toLowerCase().includes(component.toLowerCase())
      )
    );
  }
  ...
  get noFieldsToDisplay() {
    const hasNoFields =
      !this.isLoadingRecordDetail && this.displayFields.length === 0;
    console.log(
      `noFieldsToDisplay: ${hasNoFields} (isLoading=${this.isLoadingRecordDetail}, displayFieldsLength=${this.displayFields.length})`
    );

    if (hasNoFields) {
      console.log(
        "Detailed field data state:",
        JSON.stringify({
          detailedFieldsDataExists: !!this.detailedFieldsData,
          detailedFieldsDataLength: this.detailedFieldsData
            ? this.detailedFieldsData.length
            : 0,
  ...
  setIconForObject(objectApiName) {
    ...
    // For custom objects, use the custom icon with appropriate number
      ...
      const hash = Array.from(objectApiName).reduce(
  ...
  get isNewActionAvailable() {
    return (
      this.recordActionApiNames &&
      this.recordActionApiNames.toLowerCase().includes("new")
    );
  }
  ...
  get canApplyFilter() {
    ...
    if (
      ...
      this.selectedOperator === "isNotNull"
    ) {
      return true;
    }
  ...
  get effectiveLookupNavigationDisabled() {
    ...
    if (this.navigationStack.length >= this.maxNavigationDepth) {
      console.log(
        `Maximum navigation depth (${this.maxNavigationDepth}) reached, disabling lookups`
      );
      return true;
    }
  ...
  get navigationDepthClass() {
    if (
      this.maxNavigationDepth > 0 &&
      this.navigationStack.length >= this.maxNavigationDepth
    ) {
  ...
  handleFlowStatusChange(event) {
    if (
      ...
      this.fetchRecordDetails(
        this.selectedRecord.Id,
        this.selectedRecord.attributes.type
      );
  ...
  getConfiguredActions() {
    ...
    const actionNames = this.recordActionApiNames
      .split(",")
      .map((name) => name.trim());
  ...
  loadObjectActions(objectApiName, recordId) {
    ...
    const customActionNames = actionNames.filter(
      (name) => !standardActionMap[name]
    );
  ...
  handleActionClick(event) {
    ...
    if (actionName === "Edit") {
      ...
      const selectedAction = this.objectActions.find(
        (a) => a.name === actionName
      );
      ...
      if (selectedAction) {
        ...
        if (selectedAction.type === "Flow") {
          // Flow quick actions
          console.log(
            "🌊 Handling Flow action with flowApiName:",
            selectedAction.flowApiName
          );
          this.handleFlowAction(
            selectedAction.flowApiName || selectedAction.name,
            selectedAction.label
          );
        } else if (selectedAction.type === "ScreenAction") {
  ...
  handleRelatedListClick(event) {
    ...
    const selectedList = this.pageLayoutRelatedLists.find(
      (list) => list.relationshipName === relationshipName
    );
  ...
  loadRelatedListRecords(relatedList) {
    ...
    this.relatedListColumns = fields.map((fieldApiName) => {
      ...
      const label = fieldApiName.includes(".")
        ? fieldApiName
            .split(".")
            .map((part) => this.formatFieldLabel(part))
            .join(" ")
  ...
  get flowModalTitle() {
    if (this.flowApiNameToLaunch) {
      console.log(
        "🏷️ Flow modal title:",
        this.flowLabelToDisplay,
        "for flow:",
        this.flowApiNameToLaunch
      );
