...
export default class WhistleblowerSlaTracker extends LightningElement {
      ...
      this.countdownInterval = setInterval(() => {
        ...
        if (this.slaStatus === "Closed") {
          ...
          const lastModifiedDate = new Date(
            this.whistleblowerData.fields[this.lastModifiedDateFieldName].value
          );
          this.slaStatus =
            lastModifiedDate <= this.slaDeadline ? "Met" : "Breached";
          this.timeUntilSLABreach = this.formatTimeDifference(
            lastModifiedDate,
            this.slaDeadline
          );
  ...
  calculateSLA() {
    if (this.whistleblowerData && this.slaDurationHours !== undefined) {
      const createdDateValue = getFieldValue(
        this.whistleblowerData,
        `${this.objectApiName}.${this.createdDateFieldName}`
      );
      const lastModifiedDateValue = getFieldValue(
        this.whistleblowerData,
        `${this.objectApiName}.${this.lastModifiedDateFieldName}`
      );
      ...
      this.slaDeadline.setHours(
        this.slaDeadline.getHours() + parseInt(this.slaDurationHours)
      );
      ...
      switch (statusValue) {
        case "Closed":
          ...
          this.slaStatus =
            new Date(lastModifiedDateValue) <= this.slaDeadline
              ? "Met"
              : "Breached";
          break;
        default:
          ...
          this.slaStatus =
            new Date() <= this.slaDeadline ? "Active" : "Breached";
          break;
      }
