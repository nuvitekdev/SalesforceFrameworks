...
// APD fields (used when the record is APD_Application__c)
...
// Permit-side Apex for saving files into virtual folders with metadata
import uploadPermitFile from '@salesforce/apex/Nuvi_Permit_DocumentController.uploadPermitFile';

// Utility: simple base64 from Uint8Array
...
export default class PermitPrint extends NavigationMixin(LightningElement) {
  @api recordId;
  ...
  // Attempt to read APD fields if this component is placed on APD_Application__c
  ...
  renderedCallback() {
    ...
    loadScript(this, PDF_LIB)
      ...
      .catch((e) => {
        ...
        // allow retry via refresh if needed
  ...
  handleTitleChange(e) {
    this.certificateTitle = e.detail.value;
  }

  handleFolderChange(e) {
    this.targetFolder = e.detail.value;
  }
  ...
  async generatePdf() {
    if (!this.PDFLib) {
      this.showToast('Error', 'PDF library not ready', 'error');
      return;
    }
    ...
    try {
      ...
      // Header
      page.drawText(this.certificateTitle || 'Permit Certification', {
        x: margin,
        y: yStart,
        size: 20,
        font: bold,
        color: rgb(0.13, 0.74, 0.76)
      });
      ...
      let y = yStart - 40;
      lines.forEach((text) => {
        page.drawText(text, { x: margin, y, size: 12, font, color: rgb(0.11, 0.11, 0.12) });
        y -= 18;
      });
      ...
      // Certification statement (placeholder)
      const certText =
        'This certificate confirms the permit conditions and approvals as processed through the official workflow. ' +
        'By signing, the undersigned acknowledges the terms and conditions associated with this permit.';
      ...
      for (const w of words) {
        ...
        const size = font.widthOfTextAtSize(testLine, 12);
        if (size > textWidth) {
          linesOut.push(line);
          line = w;
        } else {
      ...
      linesOut.forEach((l) => {
        page.drawText(l, { x: margin, y, size: 12, font, color: rgb(0.11, 0.11, 0.12) });
        y -= 16;
      });
  ...
  downloadPdf() {
    ...
    a.href = url;
    a.download = this.fileName || 'Permit_Certificate.pdf';
    a.click();
  ...
  async saveToRecord() {
    if (!this.pdfBytes) {
      this.showToast('Error', 'Generate the PDF first', 'error');
      return;
    }
    this.isSaving = true;
    try {
      ...
      const result = await uploadPermitFile({
        applicationId: this.recordId,
        folderName: this.targetFolder || 'Application_Documents',
        fileName,
        base64Data: b64,
        contentType: 'application/pdf',
        documentType: 'PERMIT_CERTIFICATE',
      ...
      // If inline signer is present, load into it automatically
      ...
      if (signer && cvId) {
        try {
          await signer.loadPdfByContentVersionId(cvId, fileName);
        } catch (e) {
          // eslint-disable-next-line no-console
          console.error('Failed to preload signer inline', e);
        }
      }

      // Notify parent containers so they can load the signer
      if (cvId) {
        this.dispatchEvent(new CustomEvent('certificategenerated', {
          detail: { contentVersionId: cvId, fileName },
          bubbles: true,
          composed: true
        }));
      }
      ...
      // eslint-disable-next-line no-console
      console.error(e);
      const msg = e?.body?.message || e.message || 'Failed to save file';
  ...
  navigateToSigner() {
    // Option 1: Navigate to the record page where a signing component is present
    this[NavigationMixin.Navigate]({
      type: 'standard__recordPage',
      attributes: {
        recordId: this.recordId,
        objectApiName: 'APD_Application__c',
        actionName: 'view'
      }
    });
  }
